...
def get_edge_label_path(root_dir, img_name):
    return os.path.join(root_dir, EDGE_FOLDER_NAME, img_name.replace('.jpg', label_suffix))
...
class CDDataset(ImageDataset):
    ...
    def __getitem__(self, index):
        ...
        L_edge_path = get_edge_label_path(self.root_dir, self.img_name_list[index % self.A_size])

        label = np.array(Image.open(L_path), dtype=np.uint8)
        edge_label = np.array(Image.open(L_edge_path), dtype=np.uint8)
        ...
        [img, img_B], [label, edge_label] = self.augm.transform([img, img_B], [label, edge_label], to_tensor=self.to_tensor)
        ...
        return {'name': name, 'A': img, 'B': img_B, 'L': label, "L_edge": edge_label}
